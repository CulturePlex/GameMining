var QuizConstructor = {
  ...
  setQuiz: function(graph_type){
    $('.results').hide();
	console.log(QuizConstructor.value);
	WIKIPEDIA.getData('http://en.wikipedia.org/wiki/'+QuizConstructor.value.replaceAll(" ","_"),
		function (result){

			QuizConstructor.infoLeaf = result;
		    QuizConstructor.leafHtml = "<a href='#' id='leafLink'>"+QuizConstructor.value+"</a>";

			QuizConstructor.edgeHtml = QuizConstructor.attrib.replaceAll("_"," ");
			finishQuiz();
		},
		function (){
			QuizConstructor.infoLeaf = "error";
			QuizConstructor.edgeHtml = QuizConstructor.attrib.replaceAll("_"," ");
		    QuizConstructor.leafHtml = QuizConstructor.value;
			finishQuiz();
			console.log('error!');
      ...
      {
          var question = 'Which of the followings has <em>' + QuizConstructor.leafHtml +'</em> as ' + QuizConstructor.edgeHtml +'?';
      }
      ...
      else
      {
		...
		$("a#leafLink").click(function(){
			InfoManager.display(QuizConstructor.infoLeaf);
		});
        //$.unblockUI();
      }
  	};
